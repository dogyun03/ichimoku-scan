...
현재 종목 분석 창은 차트 판단을 빼고 아래 자료를 한 화면에 모읍니다.

- 실적 날짜, 최근 분기 결과, 기업 주요 통계. 실적/통계 탭은 다가오는 실적발표일 빠른 순 카드형으로 정렬
- 거래량 상위 종목의 한국어 핵심 뉴스 브리핑, 호재/악재/중립 분류, 영향도 점수, 요약, 확인 포인트
- 티커 입력형 평단가 조회와 기관·외국인·개인 수급 바로가기
- 네이버 리서치의 당일 증권사 리포트 목록, 요약, 분석
- Yahoo Finance 기반 해외 애널리스트 등급·목표가 업데이트, 상향/하향 변화 감지, 요약, 분석
- 상단 오늘 핵심 요약과 종목별 브라우저 저장형 매매 시나리오 메모

미국 종목은 네이버식 기관·외국인·개인 수급이 직접 제공되지 않으므로 검색 링크로 연결하고,
한국 6자리 종목코드는 네이버 수급 페이지로 연결합니다. 분석 창은 탭형 화면이라 맨 아래까지
스크롤하지 않고 실적/뉴스/평단/국내 리포트/해외 리포트를 바로 전환합니다. 리포트 요약은 공개 목록과
제목·메타데이터를 기준으로 만든 학습용 요약이며, 원문은 각 링크에서 확인합니다.
